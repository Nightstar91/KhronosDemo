...
public class FMODSliderParameterControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (var pair in sliderParameterPairs)
        {
            ...
            RuntimeManager.StudioSystem.setParameterByName(pair.fmodParameterName, pair.MusicSlider.value);

            // Setup the action to be performed whenever the Slider value changes.
            pair.MusicSlider.onValueChanged.AddListener((value) => UpdateFMODParameterValue(value, pair.fmodParameterName));
        }
    }
